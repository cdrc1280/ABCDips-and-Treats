<template>
  <div class="max-w-4xl mx-auto px-6 py-16">
    <div class="text-center mb-12">
      <span class="font-['Caveat'] text-brand-caramel text-2xl block mb-2">we're listening</span>
      <h1 class="text-4xl font-extrabold text-ink tracking-tight">Share Your Ideas 💡</h1>
      <p class="text-warm-gray mt-3">Help us make ABCDips & Treats even better</p>
    </div>

    <div v-if="success" class="bg-white rounded-3xl p-12 text-center border border-brand-caramel/20 shadow-lg animate-[pulse_1s_ease-in-out]">
      <div class="text-6xl mb-6 animate-bounce">🎉</div>
      <h2 class="text-2xl font-extrabold text-ink mb-2">Thank you for your suggestion!</h2>
      <p class="text-warm-gray mb-8">We appreciate your feedback and will review your ideas carefully.</p>
      <button @click="resetForm" class="bg-brand-choco text-white px-6 py-2.5 rounded-xl font-bold hover:bg-[#3D2515] transition-colors">
        Submit Another Idea
      </button>
    </div>

    <form v-else @submit.prevent="submit" class="bg-white rounded-3xl p-8 border border-brand-caramel/20 shadow-md space-y-8">
      
      <!-- Category Selection -->
      <div>
        <label class="block text-sm font-extrabold text-ink mb-3">What kind of suggestion is this? *</label>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div v-for="cat in categories" :key="cat.id" 
               ...
               :class="form.category === cat.id ? 'border-brand-choco bg-brand-tan/10 shadow-sm' : 'border-brand-caramel/20 hover:border-brand-choco/50 hover:bg-surface/50'">
            <div class="text-2xl mt-0.5">{{ cat.icon }}</div>
            <div>
              <h3 class="font-bold text-ink text-sm">{{ cat.label }}</h3>
              <p class="text-xs text-warm-gray mt-1 leading-relaxed">{{ cat.desc }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          <label class="block text-xs font-bold text-ink mb-1.5">Your Name *</label>
          <input v-model="form.name" required :readonly="authStore.isAuthenticated"
                 class="w-full bg-surface border border-brand-caramel/30 rounded-xl px-4 py-3 text-sm text-ink focus:outline-none focus:border-brand-choco transition-all disabled:opacity-70" 
                 :class="authStore.isAuthenticated ? 'bg-surface/60 cursor-not-allowed' : ''" />
        </div>
        <div>
