<template>
  <div class="page-container py-10 md:py-16">
    ...
    <div v-if="cartStore.items.length === 0" class="py-12 text-center bg-white rounded-3xl border border-[#C08E5D]/20">
      <EmptyState title="Your Basket is Empty" description="Add items to your basket before checking out.">
        <template #action>
          <RouterLink to="/shop"><BaseButton variant="primary">Return to Shop</BaseButton></RouterLink>
        </template>
      </EmptyState>
    </div>
        ...
        <div class="bg-white rounded-3xl p-6 md:p-8 border border-[#C08E5D]/20 shadow-sm space-y-4">
          ...
          <div v-if="form.fulfillment_type === 'delivery'" class="space-y-4 pt-2">
            <BaseTextarea
              ...
              placeholder="House/Unit #, Street Name, Barangay, Landmark"
              rows="3"
              required
          ...
          <BaseTextarea
            v-model="form.notes"
            label="Special Delivery / Baking Instructions (Optional)"
            placeholder="e.g. Please leave with security guard, or birthday candle count..."
            rows="2"
          />
        </div>
        ...
        <div class="bg-white rounded-3xl p-6 md:p-8 border border-[#C08E5D]/20 shadow-sm space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
              ...
              <div class="w-10 h-10 rounded-xl bg-[#5C3A22] text-[#FBF3E7] font-bold text-xs flex items-center justify-center flex-shrink-0">
                BDO
              </div>
              <div>
        ...
        <div class="bg-white rounded-3xl p-6 md:p-8 border border-[#C08E5D]/20 shadow-sm space-y-6">
          ...
          <div class="max-h-60 overflow-y-auto space-y-3 pr-1">
            <div v-for="item in cartStore.items" :key="item.id" class="flex items-center justify-between text-xs py-1">
              ...
              <span class="font-bold text-[#5C3A22] flex-shrink-0">₱{{ item.subtotal.toFixed(2) }}</span>
            </div>
          </div>
...
const form = ref({
  ...
  delivery_address: authStore.user?.address || '',
  city: 'Metro Manila',
  postal_code: '',
  notes: '',
...
async function handleCheckout() {
  ...
  try {
    ...
    router.push(`/order-confirmation?token=${data.data.tracking_token}`)
  } catch (err) {
