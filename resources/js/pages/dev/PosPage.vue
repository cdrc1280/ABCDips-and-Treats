        ...
        <div class="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div
                ...
                <div class="border-t border-[#C08E5D]/20 pt-4 space-y-4">
                    ...
                    <BaseButton variant="primary" full-width size="lg" :disabled="registerItems.length === 0"
                        :loading="processing" @click="completeCheckout">
            ...
            <div v-if="receiptOrder" class="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4">
                <div class="bg-white text-black p-6 rounded-2xl max-w-sm w-full font-mono text-xs space-y-4 shadow-2xl">
                    <div class="text-center space-y-1">
                        ...
                        <p class="text-[10px] text-gray-500">{{ new Date(receiptOrder.created_at).toLocaleString() }}
                        </p>
                    </div>
                    ...
                    <div class="space-y-1 text-right font-bold">
                        <div class="flex justify-between"><span>TOTAL:</span><span>₱{{ receiptOrder.total.toFixed(2)
                                }}</span></div>
                        <div class="flex justify-between text-gray-600"><span>METHOD:</span><span>{{
                            receiptOrder.payment_method.toUpperCase() }}</span></div>
                        <div v-if="receiptOrder.payment_method === 'cash'" class="flex justify-between text-gray-600">
                            <span>CASH:</span><span>₱{{ lastCashTendered.toFixed(2) }}</span></div>
                        <div v-if="receiptOrder.payment_method === 'cash'"
                            class="flex justify-between text-emerald-700"><span>CHANGE:</span><span>₱{{
                                lastChangeDue.toFixed(2) }}</span></div>
                    </div>
