<template>
  <div class="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
    ...
    <Transition enter-active-class="transition-all duration-300 ease-out"
      ...
      <div v-if="isOpen"
        class="w-80 bg-[#FDFBF7] dark:bg-[#1E1510] backdrop-blur-xl rounded-3xl shadow-2xl border border-[#C08E5D]/30 overflow-hidden flex flex-col"
        style="height: 490px;">
        ...
        <div class="bg-gradient-to-r from-[#4A2D1A] via-[#5C3A22] to-[#362215] px-4 py-3.5 flex items-center justify-between shrink-0 shadow-md">
          <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-full bg-[#D9A876]/30 flex items-center justify-center text-lg shadow-inner">🧁</div>
            <div>
              <p class="text-xs font-bold text-[#FBF3E7] leading-none">ABCDips AI Helper 🧁</p>
              <p class="text-[10px] text-[#E2C08A] leading-none mt-0.5 font-medium">Ask me anything about our pastries!</p>
            </div>
          </div>
          <button @click="isOpen = false" class="text-[#FBF3E7]/70 hover:text-[#FBF3E7] transition-colors p-1"
        ...
        <!-- Messages -->
        <div ref="messagesEl" class="flex-1 overflow-y-auto px-4 py-3 space-y-3 scroll-smooth relative bg-[#FDFBF7] dark:bg-[#1A120C]">
          <!-- Welcome message -->
          <div v-if="messages.length === 0" class="flex gap-2 items-start">
            ...
            <div class="bg-[#F5E8D0] dark:bg-[#2A1C13] rounded-2xl rounded-tl-sm px-3.5 py-2.5 max-w-[85%] border border-[#C08E5D]/20 shadow-xs">
              <p class="text-xs text-[#1C1410] dark:text-[#FBF3E7] leading-relaxed">Hi! I'm Dips 🧁 I can help with our pastry menu,
                ingredients, allergens, custom orders, and more. What can I get for you today?</p>
            </div>
          </div>
          ...
          <div v-for="(msg, i) in messages" :key="i"
            ...
            <div :class="[
              'rounded-2xl px-3.5 py-2.5 max-w-[85%] shadow-xs border',
              msg.role === 'user'
          ...
          <div v-if="userMessageCount < 3 && !escalationSuccess && !isGuestLimitReached" class="flex justify-center mt-4">
            <button @click="autoEscalate" class="text-xs bg-[#FBF3E7] dark:bg-[#2A1C13] border border-[#C08E5D]/40 px-3.5 py-1.5 rounded-full shadow-sm text-[#5C3A22] dark:text-[#E2C08A] font-bold hover:bg-[#E2C08A]/20 transition-all transform hover:scale-105">
              Talk to a Human 👤
            </button>
          </div>

          <!-- Escalation Success Banner -->
          <div v-if="escalationSuccess" class="bg-[#6B8F5E]/15 border border-[#6B8F5E]/30 text-[#6B8F5E] dark:text-[#A4C997] text-xs p-3 rounded-xl text-center mt-4 font-semibold">
            ✅ Chat automatically escalated to human support! Our team has received your conversation history and will respond shortly. 💬
          </div>
        ...
        <!-- Quick prompts -->
        <div v-if="messages.length === 0 && !isGuestLimitReached" class="px-3 pb-2 pt-1 flex flex-wrap gap-1.5 bg-[#FDFBF7] dark:bg-[#1A120C]">
        ...
        <div class="px-3 pb-3 pt-2 border-t border-[#C08E5D]/20 shrink-0 bg-[#FDFBF7] dark:bg-[#180E09]">
          <form @submit.prevent="sendMessage()" class="flex gap-2">
            <input v-model="inputText" :disabled="loading || isGuestLimitReached" :placeholder="isGuestLimitReached ? 'Guest limit reached — log in to chat' : 'Ask about our pastries...'"
            ...
            <button type="submit" :disabled="!inputText.trim() || loading || isGuestLimitReached"
              class="bg-[#5C3A22] text-[#FBF3E7] dark:bg-[#C08E5D] dark:text-[#1C1410] rounded-xl px-3 py-2 disabled:opacity-40 hover:bg-[#4A2D1A] transition-colors shrink-0 shadow-xs"
              v-tooltip="isGuestLimitReached ? 'Guest limit reached — please log in' : 'Send message to AI assistant'">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <button @click="isOpen = !isOpen"
      ...
      class="w-14 h-14 rounded-full bg-[#5C3A22] dark:bg-[#C08E5D] text-[#FBF3E7] dark:text-[#1C1410] shadow-xl hover:scale-110 transition-all duration-300 flex items-center justify-center relative group border border-[#C08E5D]/30">
